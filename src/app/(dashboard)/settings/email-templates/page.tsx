...
import { extractVariables } from "@/lib/email-utils";
...
const TYPE_DESCRIPTIONS: Record<string, string> = {
  welcome: "Sent when a new employee is added to the system",
  birthday: "Automated daily check — sent on employee birthday",
  anniversary: "Automated daily check — sent on hire date anniversary",
  leave_decision: "Sent when a leave request is approved or rejected",
  payslip: "Sent to each employee when payroll is marked as paid",
};
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Customize automated emails sent to employees. Use {"{variable}"} placeholders for dynamic content.
        </p>
      </div>
                      ...
                      <div className="bg-muted/50 rounded-md p-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {selectedTemplate.availableVars.split(",").map((v) => (
                            <code
                              key={v.trim()}
                              className="text-[11px] bg-background px-1.5 py-0.5 rounded border"
                            >
                              {v.trim()}
                            </code>
                          ))}
                    ...
                    <div>
                      ...
                      {showPreview ? (
                        <div className="border rounded-md overflow-hidden">
                          ...
                          <div
                            className="p-4 bg-white max-h-[500px] overflow-auto"
            ...
            <CardContent>
              {emailLogs.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">
                  No emails sent yet. Send a test email to see it here.
                </p>
              ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            To: {log.toEmail} · {log.templateType}
                          </p>
                        </div>
                      </div>
